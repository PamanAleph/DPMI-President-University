...
export default function EvaluationActions({
  ...
  const handleEditEvaluation = async () => {
    ...
    // Ensure majorResponse is typed as Major[]
    ...
    const { value: formValues } = await Swal.fire({
      ...
      focusConfirm: false,
      preConfirm: () => {
        const majorIds = selectedMajor.map((major) => major.value); // Collect all selected major IDs
        ...
        return { setupId, majorIds, semester, end_date }; // Update to return majorIds instead of majorId
      },
      didOpen: async () => {
          ...
          root.render(
            <Select
              ...
              onChange={(selectedOptions) => {
                selectedMajor = selectedOptions as { label: string; value: number }[];
              }}
            />
          );
        }
        ...
        if (setupSelectContainer) {
          ...
          root.render(
            <Select
              ...
              value={selectedSetup ? [selectedSetup] : []} // Ensure value is an array for the multi-select
              className="swal2-select"
    ...
    if (formValues) {
      try {
        await updateEvaluation({
          ...
          major_id: formValues.majorIds, // Use majorIds instead of majorId
