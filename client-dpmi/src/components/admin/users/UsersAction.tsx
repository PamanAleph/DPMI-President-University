...
export default function UsersAction({
  ...
  const handleEditEvaluation = async () => {
    ...
    const majorList = majorResponse.map(
    ...
    let selectedMajors: { label: string; value: number }[] =
      evaluation.major_id.map((majorId: number) => {
        const major = majorList.find((major) => major.id === majorId);
        return {
          label: major ? major.major_name : "Unknown Major",
          value: majorId,
        };
      });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire("Deleted!", "The evaluation has been deleted.", "success").then(
            () => {
              window.location.reload();
            }
          );
  ...
  return (
    <section className="flex gap-4 justify-center">
      ...
      <Button className="bg-red-400 text-white" onClick={handleDeleteEvaluation}>
        Delete
      </Button>
    </section>
  );
